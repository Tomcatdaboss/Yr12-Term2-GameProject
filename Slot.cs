...
public class Slot : MonoBehaviour
{
    ...
    void Update()
    {
        if (item_name == "empty"){ // if there is no item in the inventory, then call it an Empty slot
            ...
            if (isInvent){ // defines correct method of stating the item name and quantity depending on whether the slot is an equip or invent slot
                item_info.GetComponent<Text>().text = item_name + " X " + quantity;
            } else {
                item_info.GetComponent<Text>().text = item_name;
            }            
            if(item_name == "Wood"){ // essentially all these if statements do is check if the item in the slot is a specific type of item, and if so, change the item icon to the correct sprite.
